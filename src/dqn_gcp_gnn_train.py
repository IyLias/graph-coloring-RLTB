...
def set_run_name(graph_type, nodes, prob, ncolors, seed): 
    ...
    # DSJCn.x(random graph: Erdos-Renyi model) 
    if graph_type == 0:
       run_name = f"{args.env_id}__G_{nodes},{prob}_k{ncolors}"
    elif graph_type == 1:
       run_name = f"{args.env_id}__O_{nodes}__k{ncolors}"
    elif graph_type == 2:
       run_name = f"{args.env_id}__K_{nodes}__k{ncolors}"
    elif graph_type == 3:
       run_name = f"{args.env_id}__Petersen__k{ncolors}"
    elif graph_type == 4:
       run_name = f"{args.env_id}__flat{nodes}__k{ncolors}"
    elif graph_type == 5:
       run_name = f"{args.env_id}__le450_25__k{ncolors}"
    elif graph_type == 6:
       nodes = int(math.sqrt(nodes))
       run_name = f"{args.env_id}__queen{nodes}_{nodes}__k{ncolors}"
    elif graph_type == 7: 
       run_name = f"{args.env_id}__myciel{nodes}__k{ncolors}"
    elif graph_type == 8:
       run_name = f"{args.env_id}__games{nodes}__k{ncolors}"
...
"""
    ...
    # If we resume training from saved model, then load it!
    if args.resume_training and os.path.exists(args.model_path):
        saved_model = torch.load(args.model_path)
        q_network.load_state_dict(saved_model['q_network_state_dict'])
        optimizer.load_state_dict(saved_model['optimizer_state_dict'])
        target_network.load_state_dict(saved_model['target_network_state_dict'])
        processed_step = saved_model['global_step']
        print(f"Resumed training from saved checkpoint: {args.model_path} at step {processed_step}")
    ...
    for global_step in range(processed_step, processed_step+args.total_timesteps):
    ...
    if args.save_model:
        model_path = f"runs/GCP-v0_dqn_gnn/{args.exp_name}.pth"
        global_step = processed_step + args.total_timesteps 
